...
int thread_function(void *pv)
{
    ...
    while(!kthread_should_stop()) {
        printk(KERN_INFO "In Himanshu Thread Function %d\n", i++);
        msleep(1000);
    }
    return 0;
}
...
static int __init hello_driver_init(void)
{
        ...
        hello_thread = kthread_create(thread_function,NULL,"Himanshu Thread");
        if(hello_thread) {
            wake_up_process(hello_thread);
        } else {
        ...
        hello_thread = kthread_run(thread_function,NULL,"eTx Thread");
        if(hello_thread) {
